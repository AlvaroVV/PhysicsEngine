...
{
	Init();
	g_PhysicsPtr = new CPhysics( CVector3D( 0.0f, 0.198f * 2.0f, 0.0f ) );
	m_map = new CMap( m_width, m_height );
	m_numBalls = 0;
	m_fixedStep = 0.03f;
	m_physicstimeStopControl = 0.0f;
...
void CDemoApp::Init( void )
{
	...
	CreateGLWindow( L"Fisicas básicas", m_width, m_height, fullScreen );
...
void CDemoApp::Control( float timeDiff )
{
	...
	m_physicstimeStopControl += timeDiff;
	//Controlamos que la física se ejecute cada 0.03 (fixed)
	//Es simulado, quiere decir que a veces se ejecute la física varias veces seguidas para alcanzar ese lapso de 0.03.
	if (m_physicstimeStopControl >= m_fixedStep)	
	{
		int module = m_physicstimeStopControl / m_fixedStep;
		for (int i = 0; i < module; i++)
			g_PhysicsPtr->Update(m_fixedStep);
		//No lo seteamos a cero para no perder el resto de la división
		m_physicstimeStopControl -= module * m_fixedStep;
	}
	...
	//Si es click derecho comenzamos la edición de lineas.
	//Creamos el primer punto del segmento
	if (m_event.mouse.rightButton.down)
	{
		m_lineEditing = true;
		m_editP0 = CVector3D((float)m_event.mouse.x / m_width, (float)m_event.mouse.y / m_height, 0.0f);

	}
	//Para dar feedback al usuario, mientras no haya soltado el click derecho, seguimos pintando el segmento.
	if (m_lineEditing)
	{
		m_editP1 = CVector3D((float)m_event.mouse.x / m_width, (float)m_event.mouse.y / m_height, 0.0f);
	}
	//Cuando libearmos el click derecho, acabamos la edición de lineas y creamos el segmento
...
void CDemoApp::Render( void )
{
	...
	//Aquí pintamos las bolas de la app.
	for( unsigned int i=0; i<m_numBalls; ++i ) {
		m_balls[i]->Render();
	}

	//Aquí la edición de lineas mientras no se suelte el click derecho.
	if( m_lineEditing ) {
		//glLineWidth( 5.0f );
		glBegin( GL_LINES );
...
{
	//El mensaje que proviene del SO contiene información del input.
	switch( uMsg ) {
		case WM_KEYDOWN:
