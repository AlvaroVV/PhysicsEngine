...
//Estructura similar al input del raton
struct Mouse
{
...
//Nuestra aplicación se maneja solo a través del ratón
//Debemos saber cuando se pulsa y se suelta.
struct EVENT {
	Button key[256];
	Mouse mouse;
};
...
private:
	...
	float m_timeElapsed;
	float m_fixedStep;
	float m_physicstimeStopControl;
  
	// Booleano que indica si hay que mostrar la ayuda
	bool m_showHelp;

	// Indica si se ha matado la demo
	bool m_demoFinished;
